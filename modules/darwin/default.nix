...
let
  ...
  secretType = types.submodule (
    { config, ... }:
    {
      config = {
        ...
        sopsFileHash = mkOptionDefault (
          optionalString cfg.validateSopsFiles "${builtins.hashFile "sha256" config.sopsFile}"
        );
      };
      options = {
        name = mkOption {
  ...
  manifestFor =
    ...
    pkgs.writeTextFile {
      ...
      checkPhase = ''
        ${sops-install-secrets-check}/bin/sops-install-secrets -check-mode=${
  ...
  config = mkMerge [
    (mkIf (cfg.secrets != { }) {
      ...
      system.activationScripts = {
        setupSecretsForUsers = mkIf (secretsForUsers != { }) (
          stringAfter ([ "specialfs" ] ++ optional cfg.age.generateKey "generate-age-key") ''
